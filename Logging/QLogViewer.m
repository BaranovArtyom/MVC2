...
// Called in response to the UIApplicationWillResignActiveNotification.  If an action sheet is up, dismiss it per the HI guidelines.
...
// Returns a data object that holds the gz compressed contents of the log.  The resulting  data object is memory mapped to minimise the memory impact.
...
// Called in response to the user tapping the Action button.  This puts up an  alert sheet that lets the user choose what they'd like to do.
...
// Called when the action sheet goes away, where buttonIndex denotes the option  chosen by the user.
...
// Called when an alert view is dismissed.  We have to handle one special case, described  below.
...
// Called by the mail composer view when its done.  We report any errors and  then dismiss the mail composer view.
