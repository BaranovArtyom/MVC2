...
/*
 ...
 Photo represents a photo within a gallery.  It has properties like photoID, displayName, date, and so on. 
 It also has two properties that return images, thumbnailImage (the thumbnail) and photoImage (the full size image).

Photo is a managed object that comes from the Core Data database associated with the gallery.

Photo does not store the data for either the thumbnail or the full size photo.  The thumbnail data is stored in a separate 
 Thumbnail object (there's an optional one-to-one relationship between Photo and Thumbnail) and the full size photo is stored
 in a file (Photo just stores the relative path to that file).
...
// Creates a photo with the specified properties in the specified context.  The properties dictionary is keyed by property names, in a KVC fashion.
...
// Updates the photo with the specified properties.  This will update the various readonly properties listed below, triggering KVO notifications along the way.
...
// The Photo object does not download the full photo (that is, photoImage) unless someone wants to  display it.  
//Clients should register and unregister their interest in the full photo using these  methods.
