/*
  A model object that represents a gallery of photos on the network.
 
PhotoGallery represents the gallery itself.  This class has three main functions:

- It remembers the gallery URL.

- It tracks the gallery cache, creating it if necessary.  This cache includes a Core Data database and a directory that contains 
 any downloaded full size photos.

- It manages the syncing process.  When told to sync it starts a network operation (RetryingHTTPOperation) to get the XML and, 
 once that completes, it starts a parser operation (GalleryParserOperation) to parse that XML.  When that's done it looks at the
 results, which represent the new state of the gallery, and syncs that to the current state.
*/
...
@property (nonatomic, retain, readonly ) NSEntityDescription *      photoEntity;// Returns the entity description for the "Photo" entity in our database.

#pragma mark * Syncing
// These properties allow user interface controllers to learn about and control the  state of the syncing process.
...
- (void)startSync;// Force a sync to start right now.  Does nothing if a sync is already in progress.
- (void)stopSync;// Force a sync to stop right now.  Does nothing if a no sync is in progress.
