...
- (id)init{
    ...
    if (self != nil) {
        ...
        self->_runningOperationToTargetMap = CFDictionaryCreateMutable(NULL, 0, &kCFTypeDictionaryKeyCallBacks, &kCFTypeDictionaryValueCallBacks);//Creates a new mutable dictionary.
...
- (NSMutableURLRequest *)requestToGetURL:(NSURL *)url{
    ...
    [result setValue:sUserAgentString forHTTPHeaderField:@"User-Agent"];//Sets the specified HTTP header field.
...
- (void)networkRunLoopThreadEntry{
    ...
    while (YES) {
		...
		//currentRunLoop - Returns the NSRunLoop object for the current thread.
        [[NSRunLoop currentRunLoop] run];//Puts the receiver into a permanent loop, during which time it processes data from all attached input sources.
...
- (void)addOperation:(NSOperation *)operation toQueue:(NSOperationQueue *)queue finishedTarget:(id)target action:(SEL)action{
    ...
    @synchronized (self) {
        ...
        CFDictionarySetValue(self->_runningOperationToTargetMap, operation, target);//Sets the value corresponding to a given key.
...
// Called by the operation queue when the operation is done.  We find the corresponding target/action and call it on this thread.
