...
// Returns the effective run loop thread, that is, the one set by the user  or, if that's not set, the main thread.
...
- (NSSet *)actualRunLoopModes{
    ...
    if ( (result == nil) || ([result count] == 0) ) {
        result = [NSSet setWithObject:NSDefaultRunLoopMode];//Creates and returns a set that contains a single given object
    }
    return result;
}
...
// Starts the operation.  The actual -start method is very simple,  deferring all of the work to be done on the run loop thread by this  method.
