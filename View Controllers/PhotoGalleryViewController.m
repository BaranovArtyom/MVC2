...
- (id)initWithPhotoGallery:(PhotoGallery *)photoGallery{
    ...
    self = [super initWithNibName:nil bundle:nil];//Returns a newly initialized view controller with the nib file in the specified bundle.
    if (self != nil) {
        UILabel *   statusLabel;

        self->_photoGallery = [photoGallery retain];//Increments the receiver’s reference count
...
- (void)startFetcher{
    ...
    self.fetcher = [[[NSFetchedResultsController alloc] initWithFetchRequest:fetchRequest managedObjectContext:self.photoGallery.managedObjectContext sectionNameKeyPath:nil cacheName:nil] autorelease];//Returns a fetch request controller initialized using the given arguments.
    assert(self.fetcher != nil);
...
// Returns YES if there are no photos to display.  The table view callbacks use this extensively  to determine whether to show a placeholder ("No photos") or real content.
- (BOOL)hasNoPhotos{
    BOOL        result;
    NSArray *   sections;
...
// Called when the user taps the Log button.  It just presents the log  view controller.
...
// Called when the user taps the Stop button.  It just passes the command  on to the photo gallery.
- (IBAction)stopAction:(id)sender{
    #pragma unused(sender)
    [self.photoGallery stopSync];
}

// Called when the user taps the Refresh button.  It just passes the command  on to the photo gallery.
