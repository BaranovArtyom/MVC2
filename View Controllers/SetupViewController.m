    ...
    if (self != nil) {
        ...
        if (self->_choices == nil) {
            ...
            self->_choices = [[NSMutableArray alloc] initWithObjects:
				@"http://" HOSTNAME "/TestGallery/index_small.xml", 
...
+ (NSSet *)keyPathsForValuesAffectingCanSave{//???
    return [NSSet setWithObjects:@"otherChoice", @"choiceIndex", nil];// Creates and returns a set containing the objects in a given argument list
}
...
// Choose the specified row.  This updates both the UI (that is, the checkmark  accessory view) and our choiceIndex property.
...
// While there's no way to put the table view in edit mode, we still support "swipe to delete" for everything except the "other" row.
...
// As the text field in the "other" row changes, reflect that change to our  otherChoice property, which updates the Save button state via KVO.
...
- (void)presentModallyOn:(UIViewController *)parent animated:(BOOL)animated{
    ...
    navController = [[[UINavigationController alloc] initWithRootViewController:self] autorelease];//The UINavigationController class implements a specialized view controller that manages the navigation of hierarchical content. This class is not intended for subclassing.
